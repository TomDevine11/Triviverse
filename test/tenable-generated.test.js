...
const valueOf = (a) => (a.value ?? Number(a.detail.split(' ')[0]))

describe('generated Tenable catalogue', () => {
  it('produces a bounded, curated catalogue', () => {
    expect(generated.questions.length).toBeGreaterThan(90)
  })
  ...
  it('answers are ranked monotonically and the tail clears the quality floor', () => {
    for (const q of generated.questions) {
      ...
      const d = q.answers[0].detail
      const stat = d.includes('goals') ? 'goals' : d.includes('caps') ? 'caps' : d.includes('€') ? 'fee' : 'apps'
  ...
  it('the whole pool is data-backed generated questions (no hand-authored)', () => {
    expect(TENABLE_QUESTIONS.length).toBe(generated.questions.length)
    expect(TENABLE_QUESTIONS.every(q => q.id.startsWith('gen-'))).toBe(true)
  })

  it('Daily rotation is a recognisable subset; Unlimited keeps everything', () => {
    // Daily is strictly smaller — the tougher (fewer-recognisable) lists are trimmed.
    ...
    // The recognisability gate actually excludes a meaningful chunk — daily is a
    // genuine, smaller subset.
